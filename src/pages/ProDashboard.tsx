                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                    {portfolios.map((portfolio, pIdx) => {
                      ...
                      <div key={portfolio.id} className="bg-white border border-stone-200 rounded-2xl p-5 hover:border-stone-300 hover:shadow-sm transition-all">
                        ...
                        {/* A draft has never been published, so /p/:slug deliberately 404s.
                            Showing its URL — copyable, with a QR code — hands people a link
                            that says "Portfolio not found". Offer publishing instead. */}
                        {isDraft ? (
                          <div className="flex items-center justify-between gap-3 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-3">
                            <span className="text-xs text-amber-800 font-medium">
                              Journal only — no public page yet
                            </span>
                            <Link
                              to={`/create/${portfolio.template_id}`}
                              className="flex-none text-xs font-semibold text-amber-900 underline underline-offset-2 hover:text-amber-700"
                            >
                              Publish one
                            </Link>
                          </div>
                        ...
                        {/* QR code (collapsible) */}
                        <details className={`mb-3 ${isDraft ? 'hidden' : ''}`}>
