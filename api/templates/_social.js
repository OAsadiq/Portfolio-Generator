// Shared social-link icon detection for all templates.
// Detects the platform from a URL's domain and returns a matching SVG.
// Anything unrecognised falls back to a clean generic "link/website" icon.
...
// Stroke (outline) icons for non-brand destinations.
const STROKE = {
...
export function detectSocial(url) {
...
export function socialIconSvg(url, size = 18) {
  const key = detectSocial(url);
  if (STROKE[key]) {
    return `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${STROKE[key]}</svg>`;
  }
