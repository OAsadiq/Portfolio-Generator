...
function buildSocialLinks(data) {
  ...
  return `<div class="side-socials">${urls.map(url =>
    `<a href="${url}" target="_blank" rel="noopener" class="side-social" title="${detectSocial(url)}">${socialIconSvg(url, 18)}</a>`).join('')}</div>`;
}
